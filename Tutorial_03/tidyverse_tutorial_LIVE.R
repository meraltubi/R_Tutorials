...
write.csv(df.adrc.summarybysex.bind,  file="TEST.")
...
df.adrc.minmax<-df.adrc %>%
  summarise(across(everything(), list(minINUM = min, maxIMUM = max)))

tibble(df.adrc.minmax)
...
df.adrc.gender <- df.adrc %>% 
  group_by(vitals_gender) 


df.adrc.gender.nest <- df.adrc %>% 
  group_by(vitals_gender) %>% 
  nest()
